...
from devitofwi.preproc.encoding import Encoding
from devitofwi.loss.l2 import L2MultiSource
try:
...
class AcousticWave2D(NonlinearOperator):
    ...
    # def mod_allshots_multisrc(self, dt=None, indices=None, vp_true=None, encodings=None) -> NDArray:
    ...
    #             model = self._create_model(self.shape, self.origin, self.spacing, 
    #                                 self.vp, self.space_order, self.nbl, self.fs)
    #         encode = Encoding(self.source_encoding, nsrc, dobs.shape[1])
    #         encodings = encode.
    #         batch_size = self.multisource_batch_size
    #         num_batches = nsrc // batch_size
    #         if nsrc % batch_size != 0: num_batches += 1
    #         if indices is None:
    #             indices = np.arange(nsrc)
    #             np.random.shuffle(indices)
    #         all_encoding = [] if encodings is None else encodings
    #         dtot = [] # List to store each batch's output
    #         for ii, i in enumerate(tqdm(range(0, nsrc, batch_size))):
    #             batch_idx = indices[i:i+batch_size]
    #             batch_src_x = self.src[0][batch_idx]
    #             batch_src_z = self.src[1][batch_idx]
    #             # Create geometry for the current batch.
    #             geometry = self._create_geometry(model,
    #                                          batch_src_x, batch_src_z,
    #                                          self.rec[0], self.rec[1],
    #                                          self.t0, self.tn * 1.2 if self.source_encoding == "random_time_delay" else self.tn, 
    #                                          self.src_type, f0=self.f0, dt=self.dt)
    #             if self.wav is None:
    #                 src = geometry.src
    #             else:
    #                 src = CustomSource(name='src', grid=model.grid,
    #                                    wav=self.wav, npoint=batch_src_x.size,
    #                                    time_range=geometry.time_axis)
    #                 src.coordinates.data[:, :] = np.column_stack((batch_src_x, batch_src_z))
                
    #             if self.source_encoding == "random_phase":
    #                 if encodings is None:
    #                     encoding = np.random.choice([-1, 1], size=(batch_src_x.size))
    #                     all_encoding.append(encoding)
    #                 else:
    #                     encoding = all_encoding[ii]
    #                 src.data[:, :] *= encoding

    #             elif self.source_encoding == "random_time_delay":
    #                 if encodings is None:
    #                     # Let max_delay be 20% of the number of time samples in the geometry.
    #                     max_delay = int(0.2 * geometry.nt)
    #                     # For each source in the batch:
    #                     delays = np.random.randint(0, max_delay + 1, size=batch_src_x.size)
    #                     all_encoding.append(delays)
    #                 else:
    #                     delays = all_encoding[ii]
    #                 for j in range(batch_src_x.size):
    #                     d = delays[j]
    #                     # Use np.roll() to shift the column.
    #                     shifted = np.roll(src.data[:, j], d)
    #                     # Zero out the first d samples.
    #                     shifted[:d] = 0
    #                     src.data[:, j] = shifted
    ...
    #         return dtot, indices, all_encoding
    ...
    def mod_allshots_multisrc(self, dt=None, indices=None, vp_true=None, encodings=None) -> np.ndarray:
        ...
        # --- Loop Over Batches ---
        for ii, start in enumerate(tqdm(range(0, nsrc, batch_size))):
    ...
    def multi_source_loss_grad(self, vp, postprocess=None, computeloss=True, computegrad=True, 
    ...
    def gen_multi_source_grad(self, vp, postprocess=None, computeloss=True, computegrad=True):
        ...
        for isrc in tqdm(range(nsrc), desc="Computing wavefields"):
    ...
    def loss_grad(self, x, convertvp=None, postprocess=None,
        ...
        if self.multisource_batch_size == 1:
            ...
            lossgrad = self.multi_source_loss_grad(vp.reshape(self.shape),
                                                   postprocess=postprocess,
                                                   computeloss=computeloss,
                                                   computegrad=computegrad)
