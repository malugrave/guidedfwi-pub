...
Contributors:
Originally adapted from https://github.com/ajbugge/HorizonTracker
...
def create_grid(seismic_data, grid_step):
    xls=[]
    ils=[]
...
def dynamic_time_warping(seismic_data, grid, window_width):
    path_info=[]
    start = time.time()
...
def extract_horizon(seismic_data, grid, path_info, events, event_spacing_parameter):
    temporary_horizons=[]
    start = time.time()
    it=0
...
def horizon_accuracy_filter(seismic_data, input_horizons, grid, path_info,print_output, hitrate_per_trace_percent=0, hardcoded_minimum_hitrate=0):
...
def show_horizon(seismic_data, horizons, horizon_number, view, line_number, selem):
...
def regrid_horizon(seismic_data, grid, grid_step, filtered_horizons_binary, dense_grid, window_size):
...
def interpolate_horizons2d(seismic_data,unwrapped3D, horizons, dense_grid_step, phase_criterion, vertical_criterion, horizontal_criterion):
...
def interpolate_horizons3d(unwrapped3D, horizons, dense_grid_step, phase_criterion, vertical_criterion, close_neighbors):
    interpolated_horizons=[]
    from scipy import interpolate
    for horizon_number in range(0, len(horizons)):
        reflector=horizons[horizon_number]
